...
const Clothing = ({ item, onOpen }: { item: clothesType, onOpen: (item: clothesType) => void }) => {
  ...
  return (
    <div className='clothing-card w-full rounded-3xl bg-zinc-900/60 border border-white/10 hover:border-indigo-500/40 p-6 flex flex-col items-center justify-between backdrop-blur-xl shadow-xl transition-all duration-300 hover:scale-[1.02]'>
      ...
      <div className="relative w-36 h-36 my-2 flex items-center justify-center bg-zinc-950/40 rounded-2xl border border-white/5 p-2 overflow-hidden">
        {item?.image ? (
          <Image 
            alt={item.name} 
            src={item.image} 
      ...
      <button
        className='relative z-10 mt-4 w-full py-2.5 px-4 rounded-xl font-semibold text-xs bg-zinc-800 hover:bg-zinc-700 text-white border border-white/10 transition-all flex items-center justify-center gap-2 cursor-pointer'
        onClick={() => onOpen(item)}
      >
